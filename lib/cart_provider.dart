...
class CartProvider extends ChangeNotifier {
  ...
  Future<String?> submitBilling(BuildContext context) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('https://dev-blacforest.vseyal.com/api/billings'),
        headers: {
  ...
  Future<void> syncKitchenNotifications() async {
    try {
      ...
      if (cachedTables.isEmpty && normalizedBranchId.isNotEmpty && token.isNotEmpty) {
        try {
          final tablesResponse = await http.get(
            Uri.parse(
              'https://dev-blacforest.vseyal.com/api/tables?where[branch][equals]=$normalizedBranchId&limit=1&depth=1',
            ),
      ...
      String urlString =
          'https://dev-blacforest.vseyal.com/api/billings?where[status][in]=pending,ordered,confirmed,prepared&where[createdAt][greater_than_equal]=$todayStart&limit=150&sort=-createdAt&depth=2';
  ...
  Future<void> loadBillFromServer(String billId) async {
    ...
    final response = await http.get(
      Uri.parse('https://dev-blacforest.vseyal.com/api/billings/$billId?depth=3'),
      headers: {'Authorization': 'Bearer $token'},
    );
  ...
  Future<Map<String, dynamic>?> fetchCustomerData(
    ...
    try {
      ...
      final lookupUri = Uri.parse(
        'https://dev-blacforest.vseyal.com/api/billing/customer-lookup',
  ...
  Future<Map<String, dynamic>?> _getCustomerOfferSettings(
    ...
    try {
      final response = await http.get(
        Uri.parse(
          'https://dev-blacforest.vseyal.com/api/globals/customer-offer-settings?depth=0',
        ),
        headers: headers,
      );
      if (response.statusCode != 200) {
