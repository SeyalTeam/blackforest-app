...
class _OfferBannerState extends State<OfferBanner> {
  ...
  Future<void> _refreshOffersFromServer({
    ...
    try {
      final response = await http.get(
        Uri.parse(
          'https://dev1-blacforest.vseyal.com/api/globals/customer-offer-settings?depth=1',
        ),
        headers: {'Authorization': 'Bearer $token'},
      );
  ...
  String? _normalizeImageUrl(String? rawUrl) {
    ...
    if (url.startsWith('/')) {
      return 'https://dev1-blacforest.vseyal.com$url';
    }
  ...
  Future<List<Map<String, dynamic>>> _hydrateMissingOfferImages({
    ...
    try {
      final response = await http.get(
        Uri.parse('https://dev1-blacforest.vseyal.com/api/media').replace(
