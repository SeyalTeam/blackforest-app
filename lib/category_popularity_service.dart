...
class CategoryPopularityService {
  ...
  static Future<Map<String, ProductPopularityInfo>> _fetchPopularityForBranch({
    ...
    String extractCategoryId(Map<String, dynamic> item) {
      final direct = extractRefId(
        item['category'] ?? item['categoryId'] ?? item['defaultCategory'],
      );
      if (direct.isNotEmpty) return direct;

      final product = toMap(item['product']);
      ...
      final categories = toDynamicList(product['categories']);
      for (final category in categories) {
        final id = extractRefId(category);
    ...
    final countsByCategoryId = <String, double>{};
    for (final rawBill in bills) {
      ...
      for (final rawItem in items) {
        ...
        final categoryId = extractCategoryId(item);
        if (categoryId.isEmpty) continue;

        final qty = toDouble(item['quantity']);
        countsByCategoryId[categoryId] =
            (countsByCategoryId[categoryId] ?? 0) + (qty > 0 ? qty : 1.0);
      }
    }

    final popularity = <String, ProductPopularityInfo>{};
    for (final entry in countsByCategoryId.entries) {
      final count = entry.value.round();
      if (count <= 0) continue;
      final normalized = math.log(count + 1) / math.log(251);
