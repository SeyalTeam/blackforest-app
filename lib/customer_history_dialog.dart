...
class _CustomerHistoryDialogState extends State<CustomerHistoryDialog> {
    ...
    try {
      ...
      for (final field in fieldsToTry) {
        for (final phoneValue in phoneValues) {
          final uri = Uri.parse('https://dev1-blacforest.vseyal.com/api/billings')
              .replace(
                queryParameters: {
                  'sort': '-createdAt',
                  'page': page.toString(),
  ...
  Future<_ReviewedProductsLookupResult> _fetchReviewedProductKeysForPhone(
    ...
    for (final phone in phoneValues) {
      for (var page = 1; page <= 12; page++) {
        final uri = Uri.parse('https://dev1-blacforest.vseyal.com/api/reviews')
            .replace(
              queryParameters: {
                'sort': '-createdAt',
                'page': page.toString(),
  ...
  String? _resolveImageUrl(dynamic value) {
    ...
    if (value is String) {
      ...
      if (text.startsWith('/')) return 'https://dev1-blacforest.vseyal.com$text';
      if (text.contains(' ')) return null;
      return 'https://dev1-blacforest.vseyal.com/$text';
    }
    if (value is List) {
